...
std::cout<<"xx";
	...
	double mutRateS = sim_pars.mutRateS;
	double mutStepS = sim_pars.mutStepS;
	double mutRateR = sim_pars.mutRateR;
	double mutStepR = sim_pars.mutStepR;
	...
	params << "seed,N,G,c1,c2,v1,v2,w1,w2,w3,w4,m,mCorrected,interactionPartners,mutRateS,mutStepS,mutRateR,mutStepR,initS01,initS11,initR01,initR11,replicates,cauchyDist";
	params << "\n" << std::to_string(seed) << "," << std::to_string(N) << "," << std::to_string(G) << "," << std::to_string(c1) << "," << std::to_string(c2) << "," << std::to_string(v1) << "," << std::to_string(v2) << "," << std::to_string(w1) << "," << std::to_string(w2) << "," << std::to_string(w3) << "," << std::to_string(w4) << "," << std::to_string(m) << "," << std::to_string(mCorrected) << "," << std::to_string(interactionPartners) << "," << std::to_string(mutRateS) << "," << std::to_string(mutStepS)<< "," << std::to_string(mutRateR) << "," << std::to_string(mutStepR) << "," << std::to_string(initS01) << "," << std::to_string(initS11) << "," << std::to_string(initR01) << "," << std::to_string(initR11) << "," << std::to_string(replicates) << "," << std::to_string(cauchyDist),"\n";
	...
	if (computeMeansInCpp == true){
		...
		summaryStats << "rep,gen,indType,strat01,strat11,meanFit,expAlphaBeta,seed,N,G,c1,c2,v1,v2,w1,w2,w3,w4,m,mCorrected,interactionPartners,mutRateS,mutStepS,mutRateR,mutStepR,init01,init11,replicates,cauchyDist";
	}

	auto mutDistS = std::normal_distribution<double>(0.0, std::abs(mutStepS));
	auto mutDistR = std::normal_distribution<double>(0.0, std::abs(mutStepR));
	//auto MutationDistBeta = std::normal_distribution<double>(0.0, std::abs(mutStepBeta));

	if (cauchyDist == 1) {
		auto mutDistS = std::cauchy_distribution<double>(0.0, std::abs(mutStepS));
		auto mutDistR = std::cauchy_distribution<double>(0.0, std::abs(mutStepR));
...
//Random number generators
	...
	for (int rep = 1; rep <= replicates; rep++){
		...
		for (int g = 0; g < G; g++){
			...
			//Mutation
			for (int i = 0; i < N; i++){
				if (prob(rng) < mutRateS){ //Mutation occurs to s01
					double mut = mutDistS(rng);
				...
				if (prob(rng) < mutRateS){ //Mutation occurs to s11
					double mut = mutDistS(rng);
				...
				if (prob(rng) < mutRateR){ //Mutation occurs to s01
					double mut = mutDistR(rng);
			...
			if (g%reportFreq == 0 || g==(G-1)){
				if (computeMeansInCpp == false){
					...
					summaryStats << "\n" << rep << "," << g << ",Sender," << mean_s01 << "," << mean_s11 << "," << meanFit_s << "," << expAlpha  <<","<< seed<<","<<N<<","<<G<<","<<c1<<","<<c2<<","<<v1<<","<<v2<<","<<w1<<","<<w2<<","<<w3<<","<<w4<<","<<m<<","<<mCorrected<<","<<interactionPartners<<","<<mutRateS<<","<<mutStepS<<","<<mutRateR<<","<<mutStepR<<","<<initS01<<","<<initS11<<","<<replicates<<","<<cauchyDist;
					summaryStats << "\n" << rep << "," << g << ",Receiver," << mean_r01 << "," << mean_r11 << "," << meanFit_r << "," << expBeta  <<","<< seed<<","<<N<<","<<G<<","<<c1<<","<<c2<<","<<v1<<","<<v2<<","<<w1<<","<<w2<<","<<w3<<","<<w4<<","<<m<<","<<mCorrected<<","<<interactionPartners<<","<<mutRateS<<","<<mutStepS<<","<<mutRateR<<","<<mutStepR<<","<<initR01<<","<<initR11<<","<<replicates<<","<<cauchyDist;
